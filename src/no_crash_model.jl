...
NoCrashRewardModel() = NoCrashRewardModel(100.,10.,2.5,4,1.,1,2500,100)
...
function control_action(mdp::NoCrashProblem, s::Union{MLState, MLPhysicalState})
     ...
     lc = 0.0
     if is_safe(mdp,s,MLAction(acc,-mdp.dmodel.lane_change_rate)) && (s.cars[1].y != 1.0) && (s.x>1500) && (s.x<2500)
...
###############################################
    if (s.x < mdp.rmodel.target_point) && (sp.cars[1].y == mdp.rmodel.first_lane)
...
=#
    ...
    #=
    if (s.x > mdp.rmodel.target_point) && (sp.cars[1].y == mdp.rmodel.target_lane) && (s.t == 0)
           r += mdp.rmodel.reward_in_target_point
    end
    =#
